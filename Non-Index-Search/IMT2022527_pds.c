...
int put_rec_by_key( int key, void *rec )
{
	...
	*curr_offset=(int)ftell(repo_handle.pds_data_fp);
	...
	//Writing the key into the data file.
...
int get_rec_by_non_ndx_key(void *non_ndx_key, void *rec, int (*matcher)(void *rec, void *non_ndx_key), int *io_count)
{
	...
	*io_count=0;
	...
	//Comparing the records by the phone key and returning a success code if found. Returns an error status if the record is not found.
